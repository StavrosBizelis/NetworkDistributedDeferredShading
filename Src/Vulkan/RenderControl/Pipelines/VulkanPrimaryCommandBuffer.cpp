...
void VulkanPrimaryCommandBuffer::ReInit()
{
  ...
  std::deque<std::shared_ptr< VulkanPipeline > > l_newPipelines;
...
{
  std::deque<std::shared_ptr< VulkanPipeline > >::iterator l_it = m_pipelines.begin();
...
std::deque<std::shared_ptr< VulkanPipeline > > VulkanPrimaryCommandBuffer::GetPipelines(){ return m_pipelines; }
