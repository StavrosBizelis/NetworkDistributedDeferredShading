...
CCamera::CCamera()
	:m_fov(0.f), m_aspectRatio(0.f), m_nearClippingPlane(0.f), m_farClippingPlane(0.f)
{
...
void CCamera::SetViewByMouse(const int& a_middleScreenX, const int& a_middleScreenY)
{  
	int middle_x = a_middleScreenX;
	int middle_y = a_middleScreenY;
...
float CCamera::GetNearClippingPlane() const
{
	return m_nearClippingPlane;
}

float CCamera::GetFarClippingPlane() const
{
	return m_farClippingPlane;
}

FrustumData CCamera::GetFrustumData() const
{
	FrustumData l_toReturn;

	// compute width and height of the near section
	l_toReturn.m_tangent= (float)glm::tan(glm::radians(GetFov()* 0.5 ) );
	l_toReturn.m_height = GetNearClippingPlane() * l_toReturn.m_tangent;
	l_toReturn.m_width = l_toReturn.m_height * GetAspectRatio();

	// compute the Z axis of the camera referential
	// this axis points in the same direction from
	// the looking direction
	l_toReturn.m_z = glm::normalize( GetView() - GetPosition() );

	// X axis of camera is the cross product of Z axis and given "up" vector 
	l_toReturn.m_x = glm::normalize( glm::cross( l_toReturn.m_z, GetUpVector() ) );

	// the real "up" vector is the cross product of X and Z
	l_toReturn.m_y = glm::cross(l_toReturn.m_x, l_toReturn.m_z);
	return l_toReturn;
}
