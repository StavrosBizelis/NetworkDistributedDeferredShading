...
env.Append(LINKFLAGS=' /DEBUG  /PDB:NetworkDistributedDeferredShading.pdb')
...
list1=Split(' kernel32.lib glew32.lib opengl32.lib assimp.lib FreeImage.lib FreeImage.lib glew32.lib')
env.Append(LIBS=list1)



# in this list we add all the cpp files we need to compile seperated by space
list=Split('Common/Core/MatrixStack.cpp \
...
ExampleLib = env.Library( 'NetworkDistributedDeferredShading',source=list)
