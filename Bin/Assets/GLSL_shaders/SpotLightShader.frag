...
void HandleSpotLight(in SpotLight a_lightSrc, in vec3 a_camPos, in vec3 a_fragmentNormal, in vec4 a_fragmentPos, in vec3 a_fragmentSpecIntensity, in float a_fragmentSpecPower,out  vec4 a_diffuse, out  vec4 a_specular)
{
  a_diffuse = vec4(0);
  a_specular = vec4(0);
  ...
  // compute spot effect
  float l_spotEffect = dot(normalize(-a_lightSrc.m_direction), normalize(-l_lightDirection));
  if (l_spotEffect <= a_lightSrc.m_spotCutoff)
	return;
  ...
  float l_att = l_spotEffect / (a_lightSrc.m_constantAtt +   
                a_lightSrc.m_linearAtt  * l_dist +    
                a_lightSrc.m_quadraticAtt * l_dist * l_dist);   
  
  // a_ambient += l_att * vec4( a_lightSrc.m_ambient ,1) * vec4( vec3(0.2), 1);//gl_FrontMaterial.ambient;
...
void main()
{
  ...
  vec4 l_diffuse;
  vec4 l_specular;
  HandleSpotLight( ULightData, UCamPos, vTexNormal.xyz, l_fragWorldSpacePoint, vTexSpecular.xyz, vTexSpecular.a, l_diffuse, l_specular);
